...
var August2012, February2012, annotationDates, bbContext, bbFocus, canvasHeight, canvasWidth, contextLine, contextXAxis, contextXScale, contextYAxis, contextYScale, focusArea, focusLine, focusXAxis, focusXScale, focusYAxis, focusYScale, generateGraph, margin, padding, parseDate, svg;
...
generateGraph = function(dataset) {
  var allDates, allTweets, annotations, brush, brushed, candidateDate, contextFrame, date, focusFrame, focusFrameMask, point, remarkableEvents, _i, _j, _k, _len, _len1, _len2;
  allDates = [];
  allTweets = [];
  ...
  brushed = function() {
    ...
    return focusFrame.selectAll(".annotation.focus").attr("transform", function(d) {
  ...
  for (_j = 0, _len1 = dataset.length; _j < _len1; _j++) {
    ...
    for (_k = 0, _len2 = annotationDates.length; _k < _len2; _k++) {
      date = annotationDates[_k];
  ...
  annotations = focusFrameMask.selectAll("text").data(remarkableEvents).enter().append("text").attr("class", "annotation focus").attr("transform", function(d) {
    return "translate(" + (focusXScale(d.date) + padding.annotationOffset) + ", " + (focusYScale(d.tweets) + padding.annotationOffset) + ")";
  }).text(function(d) {
    var _l, _len3;
    for (_l = 0, _len3 = annotationDates.length; _l < _len3; _l++) {
      date = annotationDates[_l];
      if (d.date.getTime() === date.date) {
        return date.annotationText;
      }
    }
  });
  return annotations.on("click", function(d) {
    var _l, _len3;
    for (_l = 0, _len3 = annotationDates.length; _l < _len3; _l++) {
      date = annotationDates[_l];
      if (d.date.getTime() === date.date) {
    ...
    return focusFrame.selectAll(".annotation.focus").transition().duration(750).attr("transform", function(d) {
      return "translate(" + (focusXScale(d.date) + padding.annotationOffset) + ", " + (focusYScale(d.tweets) + padding.annotationOffset) + ")";
    });
  });
};
