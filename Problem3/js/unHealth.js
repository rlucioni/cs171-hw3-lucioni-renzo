...
var bbContext, bbFocus, canvasHeight, canvasWidth, contextLine, contextXAxis, contextXScale, contextYAxis, contextYScale, focusArea, focusLine, focusXAxis, focusXScale, focusYAxis, focusYScale, generateGraph, margin, padding, parseDate, svg;
...
generateGraph = function(dataset) {
  var allDates, allTweets, brush, brushed, contextFrame, focusFrame, focusFrameMask, point, remarkableEvents, time, _i, _j, _len, _len1;
  allDates = [];
  allTweets = [];
  ...
  for (_j = 0, _len1 = dataset.length; _j < _len1; _j++) {
    ...
    if (time === parseDate("February2012").getTime() || time === parseDate("August2012").getTime()) {
      remarkableEvents.push(point);
    }
  }
  console.log(remarkableEvents);
  return focusFrameMask.selectAll("text").data(remarkableEvents).enter().append("text").attr("class", "annotation focus").attr("transform", function(d) {
    return "translate(" + (focusXScale(d.date) + padding.annotationOffset) + ", " + (focusYScale(d.tweets) + padding.annotationOffset) + ")";
  }).text(function(d) {
